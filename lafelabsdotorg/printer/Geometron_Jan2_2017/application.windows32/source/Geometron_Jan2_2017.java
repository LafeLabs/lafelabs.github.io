...
public class Geometron_Jan2_2017 extends PApplet {
...
//     int currentCommand = key2command(key);
     if(PApplet.parseInt(key) < 0040 && PApplet.parseInt(key) > 0){
        rootMagic(PApplet.parseInt(key)); 
     }
     else{
        if(key >= ' ' && key <= '~'){
...
public void rootMagic(int localCommand){
 if(localCommand == 1){
        currentTableIndex = 0;
...
public void doTheThing(int localCommand){
    ...
    if(localCommand == 0360){//grab image
      myImage = get(PApplet.parseInt(x),PApplet.parseInt(y),PApplet.parseInt(side),PApplet.parseInt(side));
    }
    if(localCommand == 0361){//drop image
       image(myImage,x,y,PApplet.parseInt(side),PApplet.parseInt(side));
    }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Geometron_Jan2_2017" };
    if (passedArgs != null) {
